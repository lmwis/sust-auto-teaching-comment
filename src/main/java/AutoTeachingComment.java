...
//        Map<String, Object> userCourseQueryParams = packageUserCourseQueryParams();
        String userCourseGuide = HttpRequest.post(COMMENT_LIST_URL).execute().body();
        System.out.println(userCourseGuide);
        for(String str : execRegxGroups(userCourseGuide,evaluationLessonRegex)){
            String evaluationLessonId = str.substring(0,str.indexOf("&"));
            String teacherId = str.substring(str.indexOf("="));
            HttpRequest.post(COMMENT_SUBMIT_URL)
